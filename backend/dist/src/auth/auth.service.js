...
let AuthService = class AuthService {
    ...
    validateEmail(email) {
        if (!email || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
            ...
            throw new common_1.BadRequestException(ERROR_MESSAGES.EMAIL_INVALID);
        }
    }
    validatePassword(password) {
        if (!password || password.length < MIN_PASSWORD_LENGTH) {
            console.error(`🚨 [AuthService] Invalid password length: ${password ? password.length : 'undefined'}`);
            throw new common_1.BadRequestException(ERROR_MESSAGES.PASSWORD_INVALID);
        }
    }
    validateOtp(otp) {
        if (!otp || otp.length !== OTP_LENGTH) {
            console.error(`🚨 [AuthService] Invalid OTP format: ${otp}`);
            throw new common_1.BadRequestException(ERROR_MESSAGES.OTP_INVALID);
        }
    }
    validateName(name) {
        if (!name || name.trim().length === 0) {
            console.error(`🚨 [AuthService] Name is required but was: ${name}`);
            throw new common_1.BadRequestException(ERROR_MESSAGES.NAME_REQUIRED);
        }
    }
    async sendEmail(to, subject, text, html, context) {
        try {
            ...
            throw new common_1.BadRequestException(ERROR_MESSAGES.EMAIL_SEND_FAILED);
        }
    }
    async signup(email, password, name, role = client_1.Role.USER) {
        try {
            ...
            if (existingUser) {
                if (!existingUser.isEmailVerified && existingUser.otpExpires && existingUser.otpExpires < new Date()) {
                    ...
                    throw new common_1.BadRequestException(ERROR_MESSAGES.EMAIL_EXISTS);
                }
                else {
                    console.error(`🚨 [AuthService] Email has a pending verification: ${email}`);
                    throw new common_1.BadRequestException(ERROR_MESSAGES.EMAIL_PENDING);
                }
            }
      ...
      `;
            ...
            throw new common_1.BadRequestException(error.message || ERROR_MESSAGES.SIGNUP_FAILED);
        }
    }
    async getOtp(email) {
        try {
            ...
            if (!user) {
                ...
                throw new common_1.BadRequestException(ERROR_MESSAGES.EMAIL_NOT_FOUND);
            }
            if (user.isEmailVerified) {
                console.error(`🚨 [AuthService] Email already verified: ${email}`);
                throw new common_1.BadRequestException(ERROR_MESSAGES.OTP_NOT_FOUND);
            }
            if (user.otpExpires && user.otpExpires < new Date()) {
                console.error(`🚨 [AuthService] OTP expired for email: ${email}`);
                await this.prisma.user.delete({ where: { email } });
                throw new common_1.BadRequestException(ERROR_MESSAGES.OTP_EXPIRED);
            }
            ...
            throw new common_1.BadRequestException(error.message || ERROR_MESSAGES.OTP_NOT_FOUND);
        }
    }
    async login({ email, password }) {
        try {
            ...
            if (!user.isEmailVerified) {
                ...
                throw new common_1.BadRequestException(ERROR_MESSAGES.CREDENTIALS_INVALID);
            }
            ...
            throw new common_1.BadRequestException(error.message || ERROR_MESSAGES.LOGIN_FAILED);
        }
    }
    async verifyOtp(email, otp) {
      ...
      `;
            await this.sendEmail(email, `${EMAIL_SUBJECTS.WELCOME}, ${user.name || 'User'}!`, welcomeText, welcomeHtml, 'welcome');
            ...
            throw new common_1.BadRequestException(error.message || ERROR_MESSAGES.OTP_VERIFY_FAILED);
        }
    }
    async forgotPassword(email) {
      ...
      `;
            ...
            throw new common_1.BadRequestException(error.message || ERROR_MESSAGES.RESET_FAILED);
        }
    }
